...
export class Merger { 
    ...
    async merge(): Promise<void> {
        ...
        const commitStatuses = await retry.exec(
            async (count): Promise<ReposListCommitStatusesForRefResponseData> => {
                let commitStatuses: ReposListCommitStatusesForRefResponseData  = Object()
                try {
                    const {data: commitStatuses} = await client.repos.listCommitStatusesForRef({
                        owner, 
                        repo,
                        ref,
                    })
